<div class="flex flex-col justify-center items-center mt-14 mb-8 h-auto m-5 gap-3 w-auto">
    ...
    <div class="container max-w-[900px]">
        ...
        <?php
        $title = count($horarios) ? 'Edite' : 'Cadastre';
        ?>

            <h1 class="dark:text-amber-300 text-center m-14 text-3xl"><?= $title ?> seu horário de funcionamento</h1>
            ...
            <div class="mt-2 md:mt-5 w-auto md:w-auto md:gap-5 mx-auto m-8 text-base flex flex-wrap justify-center gap-4 flex-col md:flex-row items-center">
                ...
                for ($i = 1; $i <= 7; $i++) {
                    ...
                    $startTime = $horario ? $horario->getHoraInicio() : '';
                    $endTime = $horario ? $horario->getHoraFim() : '';
                ...
                ?>
                    <div class="card border border-gray-300 rounded-lg p-6 mb-4 w-64 h-auto bg-gray-50 dark:bg-zinc-900 hover:">
                        ...
                        <div class="mb-4">
                            <label for="<?= $startTimeName ?>" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Horário de início:</label>
                            <div class="relative">
                                <input type="time" id="<?= $startTimeName ?>" name="<?= $startTimeName ?>" value="<?php echo $startTime ?>"
                        ...
                        <div>
                            <label for="<?= $endTimeName ?>" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Horário de fim:</label>
                            <input type="time" id="<?= $endTimeName ?>" name="<?= $endTimeName ?>" value="<?php echo $endTime ?>"
